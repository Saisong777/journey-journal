...
- **Trip Notes (出團說明):** Dynamic trip briefing notes system. Admin can create/edit/delete notes in "說明管理" (`/admin/trip-notes`), then assign them to specific trips via checkboxes in "旅程管理". Users see assigned notes in the "出團說明" tab of the Tools page (`/tools`). Tables: `trip_notes` (id, title, content) and `trip_note_assignments` (tripId, noteId, sortOrder). Endpoints: `GET/POST/PATCH/DELETE /api/admin/trip-notes`, `GET/POST/DELETE /api/admin/trips/:tripId/notes`, `GET /api/trips/current/notes`. Sample data seeded in startup migration.
