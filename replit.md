...
- **Database Schema:** Standardized schema including `users`, `profiles`, `trips`, `groups`, `user_roles`, `journal_entries`, `journal_photos`, `devotional_entries`, `evening_reflections`, `devotional_courses`, `trip_invitations`, `auth_tokens`, `platform_roles`, `trip_notes`, `trip_note_assignments`, and `bible_verses` tables.
