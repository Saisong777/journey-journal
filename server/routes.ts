...
const updateProfileSchema = z.object({
  ...
  familyMembers: z.string().max(2000).optional().nullable(),
  ...
  app.post("/api/admin/trips/:tripId/attractions/import", requireAdmin, async (req, res) => {
    try {
      ...
      const data = items.map((item) => ({
        ...
        nameAlt: str(item.nameAlt ?? item.name_alt),
        ...
        scriptureRefs: str(item.scriptureRefs ?? item.scripture_refs),
        bibleBooks: str(item.bibleBooks ?? item.bible_books),
        storySummary: str(item.storySummary ?? item.story_summary),
        keyFigures: str(item.keyFigures ?? item.key_figures),
        historicalEra: str(item.historicalEra ?? item.historical_era),
        theologicalSignificance: str(item.theologicalSignificance ?? item.theological_significance),
        lifeApplication: str(item.lifeApplication ?? item.life_application),
        discussionQuestions: str(item.discussionQuestions ?? item.discussion_questions),
        archaeologicalFindings: str(item.archaeologicalFindings ?? item.archaeological_findings),
        historicalStrata: str(item.historicalStrata ?? item.historical_strata),
        accuracyRating: str(item.accuracyRating ?? item.accuracy_rating),
        keyArtifacts: str(item.keyArtifacts ?? item.key_artifacts),
        tourRoutePosition: str(item.tourRoutePosition ?? item.tour_route_position),
        bestTime: str(item.bestTime ?? item.best_time),
        dressCode: str(item.dressCode ?? item.dress_code),
        photoRestrictions: str(item.photoRestrictions ?? item.photo_restrictions),
        crowdLevels: str(item.crowdLevels ?? item.crowd_levels),
        safetyNotes: str(item.safetyNotes ?? item.safety_notes),
        accessibility: str(item.accessibility),
        nearbyDining: str(item.nearbyDining ?? item.nearby_dining),
        accommodation: str(item.accommodation),
        nearbyBiblicalSites: str(item.nearbyBiblicalSites ?? item.nearby_biblical_sites),
        localProducts: str(item.localProducts ?? item.local_products),
        recommendationScore: str(item.recommendationScore ?? item.recommendation_score),
        physicalComment: str(item.physicalComment ?? item.physical_comment),
      }));
