...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      imgSrc: ["'self'", "data:", "blob:", "https://*.tile.openstreetmap.org", process.env.R2_PUBLIC_URL || `https://${process.env.R2_BUCKET_NAME}.${process.env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com`].filter(Boolean),
      connectSrc: ["'self'", "https://accounts.google.com", "https://*.tile.openstreetmap.org", `https://${process.env.R2_BUCKET_NAME}.${process.env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com`, process.env.R2_PUBLIC_URL].filter(Boolean) as string[],
